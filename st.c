...
char *getsel(void) {
    ...
    for (y = sel.nb.y; y <= sel.ne.y; y++) {
         ...
         */
        if ((y < sel.ne.y || lastx >= linelen) && !(last->mode & ATTR_WRAP)) *ptr++ = '\n';
    }
    *ptr = 0;
    return str;
}
...
void tputc(Rune u) {
    ...
    if (term.esc & ESC_STR) {
        ...
        if (term.esc & ESC_DCS && strescseq.len == 0 && u == 'q') term.mode |= MODE_SIXEL;
    ...
    while (passes != term.bot + 2) {
        ...
        */
        for (col = 0, i = 0; col < colend; ++col, ++i) {
            /* assume ascii */
            if (term.line[row][col].u > 127) continue;

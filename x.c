...
int xsetcolorname(int x, const char *name) {
    ...
    dc.col[x] = ncolor;
    if (x == defaultbg) dc.col[defaultbg].color.alpha = (unsigned short)(0xffff * alpha);
...
void xdrawcursor(int cx, int cy, Glyph g, int ox, int oy, Glyph og, Line line, int len) {
    ...
    if (IS_SET(MODE_FOCUSED)) {
        ...
        XftDrawRect(xw.draw, &drawcol, borderpx + cx * win.cw, borderpx + cy * win.ch, win.cw - 1,
                    1);
        XftDrawRect(xw.draw, &drawcol, borderpx + cx * win.cw, borderpx + cy * win.ch, 1,
                    win.ch - 1);
        XftDrawRect(xw.draw, &drawcol, borderpx + (cx + 1) * win.cw - 1, borderpx + cy * win.ch, 1,
                    win.ch - 1);
        XftDrawRect(xw.draw, &drawcol, borderpx + cx * win.cw, borderpx + (cy + 1) * win.ch - 1,
                    win.cw, 1);
    }
}
        ...
        for (i = 0; i < 256; i++) {
            ...
            if (!XrmGetResource(xrdb, loadValue, loadValue, &type, &ret)) {
                ...
                if (!XrmGetResource(xrdb, loadValue, loadValue, &type, &ret))
                    ...
                    if (i > 15) colorname[i] = NULL;
            }

            if (ret.addr != NULL && !strncmp("String", type, 64)) colorname[i] = ret.addr;
        }
